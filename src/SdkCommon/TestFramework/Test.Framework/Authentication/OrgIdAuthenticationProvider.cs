...
{
    public class OrgIdAuthenticationProvider : AuthenticationProvider
    {
        protected override bool TryDoAuthentication(TestEnvironment env, IDictionary<string, string> settings, ExecutionMode mode, out AuthorizationContext context)
        {
            context = null;
            if (settings == null ||
                !settings.ContainsKey(TestEnvironment.AADPasswordKey))
            {
                return false;
            }

            context = TokenCloudCredentialsHelper.GetUserToken(
                env.Endpoints.AADAuthUri.ToString(),
                env.Tenant,
                env.ClientId,
                env.ManagementResource,
                env.Endpoints.GraphUri.ToString(),
                env.UserName,
                settings[TestEnvironment.AADPasswordKey]);
